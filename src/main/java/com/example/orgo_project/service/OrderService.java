...
public class OrderService implements IOrderService {
    ...
    public List<OrderSummaryDTO> getMyOrders(Integer accountId) {
        return orderRepository.findByUserIdOrderByOrderedAtDesc(accountId).stream().map(this::toSummary).toList();
    }
    ...
    public OrderDetailDTO getOrderDetail(Integer accountId, Integer orderId) {
        ...
        if (!order.getUserId().equals(accountId)) throw new RuntimeException("Không có quyền xem đơn này");
    ...
    public boolean cancelOrder(Integer accountId, Integer orderId, String reason) {
        CustomerOrder order = orderRepository.findById(orderId).orElseThrow(() -> new RuntimeException("Không tìm thấy đơn hàng"));
        if (!order.getUserId().equals(accountId)) throw new RuntimeException("Không có quyền hủy đơn này");
        if (order.getOrderStatus() != OrderStatus.PENDING && order.getOrderStatus() != OrderStatus.PROCESSING) throw new RuntimeException("Chỉ được hủy đơn ở trạng thái PENDING hoặc PROCESSING");
    ...
    private OrderSummaryDTO toSummary(CustomerOrder order) {
        return OrderSummaryDTO.builder().id(order.getId()).orderCode(order.getOrderCode()).orderStatus(order.getOrderStatus() != null ? order.getOrderStatus().name() : null).paymentStatus(order.getPaymentStatus() != null ? order.getPaymentStatus().name() : null).totalAmount(order.getTotalAmount()).orderedAt(order.getOrderedAt()).build();
    }

    private OrderItemDTO toItemDTO(CustomerOrderItem item) {
        return OrderItemDTO.builder().id(item.getId()).productVariantId(item.getProductVariantId()).quantity(item.getQuantity()).unitPrice(item.getUnitPrice()).lineTotal(item.getLineTotal()).build();
    }
}
